...
@Service
public class EuroNewsRussiaClient implements NewsClient {

    private static final DateTimeFormatter formatter = DateTimeFormatter.ofPattern("/yyyy/MM/dd");

    @Value("${news.euronews-russia.base-url}")
    private String url;
        ...
        for (int i = 0; i < pageCount; i++) {
            ...
            List<Article> list = document.select("article .m-object__title__link[rel=\"bookmark\"]").stream()
                    .map(element -> new Article()
                            ...
                            .setSource(ArticleSource.EURONEWS_RUSSIA)
    ...
    private Article toArticle(EuronewsArticle article) {
        return new Article()
                .setSource(ArticleSource.EURONEWS_RUSSIA)
                .setTitle(article.getTitle())
                .setUrl(url + article.getFullUrl())
